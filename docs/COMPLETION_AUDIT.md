...
| Cover local validation and limit helper contracts | `tests/e2e/helper-rails.spec.ts` verifies AI task input validation/defaulting, API-key scope mapping, quota response payloads, and rate-limit response headers | Done |
...
| Harden core task mutation routes | `lib/task-validation.ts` validates and normalizes task-create payloads before quota/insert; `PATCH /api/tasks/[id]` uses the same shared rail to allowlist user-editable fields and reject protected/server-managed fields such as `user_id`, `completed_at`, `source_agent_id`, and `agent_output`; `DELETE /api/tasks/[id]` returns 404 when no owned task is deleted; e2e covers unauthenticated/config guardrails and focused helper tests cover create/patch validation contracts | Done |
