...
| Harden Stripe plan selection and entitlement mapping | `/api/stripe/checkout` now accepts only `pro` or `power` plan keys and derives price IDs from server env; webhooks skip unknown Stripe prices instead of defaulting to paid access; `tests/e2e/stripe-entitlements.spec.ts` verifies missing, placeholder, unknown, and configured price IDs; e2e covers unsupported checkout plans | Done |
