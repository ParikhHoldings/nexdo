...
| Keep authenticated task capture honest on save failure | Authenticated task creation no longer falls through to a local demo task when `/api/tasks` fails; the input is restored and the error is shown | Done |
