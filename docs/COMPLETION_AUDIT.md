...
| Keep authenticated task capture honest on save failure | Authenticated task creation no longer falls through to a local demo task when `/api/tasks` fails or Supabase returns no active user for a stale authenticated store; the input is restored and the error is shown | Done |
| Keep briefing aligned with task changes | Logged-out daily briefing refreshes from local task state after demo task capture/import; authenticated briefing now refetches when task state or user name changes; Playwright verifies the demo active-task count updates after adding a task | Locally verified for demo; implemented locally for authenticated |
...
| Verify demo workspace navigation and task lifecycle | Playwright now covers `/all` search/filtering, `/upcoming` date grouping, completing a task into `/done`, persistence after reload, and clearing completed demo tasks; authenticated Done-page bulk delete failures restore failed tasks locally and show a visible error | Locally verified for demo; implemented locally for authenticated failure rollback |
...
| Keep task mutation failures visible | Authenticated task edit/delete failures now roll back optimistic local state and surface task-store errors through the app toast layer instead of console-only failures | Done |
...
| Harden authenticated agent execution | `/api/agent/execute` now requires a `taskId`, loads the owned task from Supabase, rejects non-executable task types, maps quota pre-check and post-run accounting failures through the shared quota status helper, records quota usage after a successful run, and only then saves `agent_output` server-side; generic task PATCH rejects client-supplied `agent_output` | Done |
| Enforce task quotas on imports | CSV, JSON, ICS, Todoist, Google Tasks, and Microsoft To Do imports now pre-check requested task quota, save imported tasks, then consume quota only for successfully inserted rows; inserted rows are returned with IDs so quota-accounting failures can clean them up; migration `004_quota_noop_audit_cleanup.sql` prevents quota read probes from writing zero-quantity audit noise | Done |
...
| Cover DB-backed MCP tool handlers and API-key validation locally | `tests/e2e/mcp-tool-handlers.spec.ts` exercises owned list/search/get/briefing behavior, create-task idempotent replay, quota pre-check, parse/insert/post-insert quota ordering, cleanup on quota-accounting failure, update/complete mutations, validation failures, `agent_action_events` audit logging, hashed API-key lookup, legacy raw-key migration, and paid-plan API access gating through an in-memory Supabase double | Done |
...
- `npm run typecheck`
- `npm run build`
- `npm run test:e2e` including public landing/signup demo CTA smoke, logged-out task capture/edit/reload persistence/agent output, agent output history/review notes, demo briefing refresh, local-date due-today behavior, All Tasks search/filtering, Upcoming grouping, Done clear/reload lifecycle, file-import preview/confirm flow, persistent appearance and browser reminder settings, auth guardrails, MCP/OpenAPI/Stripe guardrails, DB-backed MCP handler and API-key validation coverage, demo CSV file import, login/signup demo-mode entry, free-plan API-key UI gating, settings tab deep links, Connect AI API-tab guidance, agent-execution quota status mapping, profile-update miss handling, and focused task route validation contracts
