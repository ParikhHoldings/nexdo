...
| Align Connect AI setup with paid API gate | `/settings/mcp` now shows the Power-plan API access requirement before users try to generate/connect keys, disables the connection tester until API access and a key exist, and clarifies that setup clients need the full one-time key rather than the stored key hint; Playwright verifies the billing deep link and disabled free-plan tester | Done |
