...
| Harden core task mutation routes | `lib/task-validation.ts` validates and normalizes task-create payloads before quota/insert; `POST /api/tasks` pre-checks quota, inserts the owned task, then records usage with best-effort cleanup if accounting fails; `PATCH /api/tasks/[id]` uses the same shared rail to allowlist user-editable fields and reject protected/server-managed fields such as `user_id`, `completed_at`, `source_agent_id`, and `agent_output`; `DELETE /api/tasks/[id]` returns 404 when no owned task is deleted; e2e covers unauthenticated/config guardrails and focused helper tests cover create/patch validation contracts | Done |
