...
| Harden MCP tool input, search, and quota rails | MCP handlers validate bounded IDs, refs, metadata, query, task updates, and status/priority values; `search_tasks` searches title, context, and tags as advertised; `create_task` consumes task-create quota after idempotency replay checks and before insert; OpenAPI exposes key length limits | Done |
