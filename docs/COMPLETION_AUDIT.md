...
- `npm run typecheck`
- `npm run build`
- `npm run test:e2e` including logged-out task capture/edit/reload persistence/agent output, agent output history/review notes, demo briefing refresh, local-date due-today behavior, All Tasks search/filtering, Upcoming grouping, Done clear/reload lifecycle, file-import preview/confirm flow, persistent appearance and browser reminder settings, auth guardrails, MCP/OpenAPI/Stripe guardrails, DB-backed MCP handler and API-key validation coverage, demo CSV file import, login demo-mode entry, free-plan API-key UI gating, and settings tab deep links
