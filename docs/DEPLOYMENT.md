...
## Current status
PR #3 passed the GitHub Actions Web rails workflow and Vercel preview deployment on head `6e196ed49a5445f404494599f2f4e7af5801d185`; the preview URL was `https://ph-nexdo-39vxlvbl6-nathan-happywpcos-projects.vercel.app`. Direct remote route smoke against that protected preview still fails at desktop `/` because Vercel Deployment Protection is active until `VERCEL_AUTOMATION_BYPASS_SECRET` is supplied locally or an unprotected preview URL is used. Local `npm run verify:env` currently fails because `.env.local` is absent; only `.env.local.example` exists in this workspace. No production deploy target or production provider credentials were verified in this operating pass. Treat Nexdo as locally, CI, and preview-build verified on the inspected head, with rendered remote route smoke and production readiness still blocked until the checks below pass against the real deployment environment.
