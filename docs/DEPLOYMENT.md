...
Write Supabase smoke that creates and deletes disposable auth/task/note/audit data and verifies profile grants, profile insert denial, profile content-bound denial, task source-spoof denial, task content-bound denial, task and task-note column grants, audit privacy, Stripe event record privacy/write denial, uniqueness, quota, and rate limits:
