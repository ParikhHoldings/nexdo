...
- Supabase: apply migrations to a real project, create a user, verify profile creation, RLS, task CRUD, task-note CRUD and length limits, import quota enforcement, hashed API-key storage, API key scope persistence and rotation rate limits, profile column read/update grants, direct task source-spoof denial, direct task content-bound denial, task and task-note column-grant denial for server-managed fields, agent external-ref uniqueness, `agent_action_events` audit writes/privacy, quota no-op behavior, and service-role RPCs.
...
Write Supabase smoke that creates and deletes disposable auth/task/note/audit data and verifies profile grants, task source-spoof denial, task content-bound denial, task and task-note column grants, audit privacy, uniqueness, quota, and rate limits:
