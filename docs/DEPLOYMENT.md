...
## Current status
As of the 2026-05-17 inspection, PR #3 passed the GitHub Actions Web rails workflow on current pushed product head `537c04b8b92b6fff38bb1519fe016e3fa7f0a20b`. Vercel preview deployment passed on the earlier inspected due-time scan-ordering head `1d12864044597dc7202ab068bc423f9ba25cf667`, but the latest Vercel status on `537c04b8b92b6fff38bb1519fe016e3fa7f0a20b` is account build-rate-limited, so that head has no fresh preview-deploy evidence. The latest checked Vercel branch preview URL was `https://ph-nexdo-git-codex-launch-rea-42bdec-nathan-happywpcos-projects.vercel.app`. Direct remote route smoke against that protected preview still fails at desktop `/` because Vercel Deployment Protection is active until `VERCEL_AUTOMATION_BYPASS_SECRET` is supplied locally or an unprotected preview URL is used. Local `npm run verify:env` currently fails because `.env.local` is absent; only `.env.local.example` exists in this workspace. No production deploy target or production provider credentials were verified in this operating pass. Treat Nexdo as locally and CI verified on the inspected product head, with fresh preview-build evidence, rendered remote route smoke, and production readiness still blocked until the checks below pass against the real deployment environment.
...
- MCP/ChatGPT Actions: provision real scoped API keys or provide existing smoke keys, then run authenticated SSE endpoint discovery, malformed JSON-RPC/action payload guard checks, and list/create/update/add-note/complete/search/briefing/get-task calls against real task data, including `create_task` replay with a repeated `source_agent_id` plus `external_ref`, `complete_task` task-row trace persistence, `add_task_note` `agent_result` note readback, and `agent_action_events` audit rows. For protected Vercel previews, export `VERCEL_AUTOMATION_BYPASS_SECRET` first.
...
Write Supabase smoke that creates and deletes disposable auth/task/note/audit data and verifies profile grants, profile insert denial, profile content-bound denial, task source-spoof denial, task relationship write denial, task content-bound denial, task and task-note column grants, audit privacy, Stripe event record privacy/write denial, usage-event mutation denial, rate-limit bucket privacy, daily briefing cache write denial, uniqueness, quota, and rate limits:
