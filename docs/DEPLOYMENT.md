...
- Supabase: apply migrations to a real project, create a user, verify profile creation, RLS, task CRUD, import quota enforcement, hashed API-key storage, API key scope persistence and rotation rate limits, profile column-update grants, `agent_action_events` audit writes, quota no-op behavior, and service-role RPCs.
