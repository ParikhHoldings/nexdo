...
- `npm run smoke:mcp` exists for real API-key SSE endpoint discovery, list/search/briefing/get/update/complete/idempotency checks, ChatGPT Actions list/search response-shape checks, optional read-only scope denial, provisioned disposable scoped keys, and required `agent_action_events` audit verification with `--provision --write --audit` once a real environment is configured.
