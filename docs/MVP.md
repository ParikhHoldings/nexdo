...
- Stripe test-mode checkout, portal, signed webhook, entitlement, quota, and idempotency smoke. `npm run smoke:stripe -- --write --webhook` now covers signed webhook delivery, unknown-price fail-closed behavior, free/pro/power tier transitions, quota plan-state boundaries, and duplicate webhook replay; an authenticated quota-consuming app action still needs to be exercised against the resulting test-mode plan state.
