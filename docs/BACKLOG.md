...
- run `npm run smoke:supabase -- --write` against a real Supabase project after applying migrations, including hashed API-key columns, profile column read/update grants and content-bound denials, direct task and task-note column-grant denials, audit-event privacy, and agent external-ref uniqueness
