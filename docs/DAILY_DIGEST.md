...
- Verified PR #3 Web rails on inspected launch-readiness heads; Vercel preview evidence remains per-head, with both successful preview deployments and account build-rate-limited statuses observed during the launch-readiness pass.
...
- Expanded MCP `update_task`, OpenAPI, local handler coverage, and the provider smoke path so agents can update owned parent/related task links through the same relationship ownership checks used by task detail.
...
- Added Settings data export so currently loaded tasks can be downloaded as JSON or CSV for backup, review, or agent handoff.
