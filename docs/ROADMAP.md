...
- `npm ci`, `npm run lint`, `npm run typecheck`, `npm run build`, `npm run test:e2e` with 97 tests, dependency audit, and the technical launch smoke partial passed locally on 2026-05-17.
...
- verify Supabase migrations, RLS, auth, demo-mode fallback, profile insert/read/update grants and content bounds, task/task-note column grants and content bounds, audit privacy, and profile/task flows
