...
- PR #3 Web rails passed on inspected heads in this pass; inspect current checks after each push before treating the newest head as current-green.
...
- Vercel preview deployment is volatile by head: one inspected docs-refresh head passed and a later docs-only head hit the known account build-rate limit.
