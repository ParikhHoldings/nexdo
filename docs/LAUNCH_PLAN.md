...
### Must ship
- A local app that passes clean install, lint, typecheck, build, dependency audit, and Playwright smoke tests. Latest full local rail passed 93 tests on 2026-05-17.
...
- The latest Vercel preview deploy is green, but remote route smoke needs `VERCEL_AUTOMATION_BYPASS_SECRET` or an unprotected preview URL because the preview is currently behind Vercel Deployment Protection.
