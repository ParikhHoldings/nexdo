...
## 2026-05-17 - Copied task handoffs can be restored through import
### Decision
`/import` should accept copied `# Nexdo Task Handoff` briefs from task detail and restore them into a new task with structured planning metadata plus reviewable notes.
...
### Decision
`npm run smoke:mcp` should call the authenticated MCP initialized notification, SSE discovery endpoint, malformed JSON-RPC and ChatGPT Action payload guard checks, actual MCP JSON-RPC and ChatGPT Actions read surfaces, and its write mode should create, replay, read, update, add a task note, complete, and optionally audit a disposable task. MCP `complete_task` and `add_task_note` accept optional source-agent metadata so completion and note events can be traced like create and update events. `complete_task` persists source-agent metadata on the completed task row as well as the audit event, so human task surfaces can identify externally completed work.
