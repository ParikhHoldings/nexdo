...
- Vercel preview deployment passed on inspected head `a9eabf66edc12cee1382d725407c4f591bcb6275`; earlier inspected heads also passed, while some intermediate heads hit the known account build-rate-limit status. Recheck PR status after every newer push because preview evidence remains per-head.
...
- Task relationships: validation lives in `lib/task-relationships.ts`; authenticated relationship writes use `app/api/tasks/[id]/relationships/route.ts` with service-role persistence after checking all linked task IDs are owned by the same user. Keep direct browser grants for `parent_task_id` and `related_task_ids` closed unless this ownership model changes deliberately.
