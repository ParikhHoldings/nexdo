...
- PR #3 GitHub Actions Web rails and Vercel preview evidence are per-head. Inspect current PR checks after every push before treating the branch as current-green; inspected heads have included both successful preview deployments and Vercel account build-rate-limit failures.
