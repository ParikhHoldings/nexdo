...
- Task handoffs: `lib/task-handoff.ts` formats and parses copied `# Nexdo Task Handoff` briefs; task detail owns copy-out, and `/import` owns paste-in restore into task metadata plus reviewable notes. Do not write source-agent trace fields directly from browser import paths; preserve them as handoff trace notes unless the server/API-key path is explicitly doing an audited agent mutation.
