...
- All Tasks agent review filtering for separating unreviewed/needs-revision outputs and traceable agent-created or agent-updated tasks from verified outputs
- scan-view task cards surface agent-output review badges so unreviewed, needs-revision, and verified outputs are visible before opening task detail
- sidebar Agent Review navigation deep-links to `/all?review=needs_review` and shows a count of reviewable agent outputs plus traceable agent task mutations
...
- local-date-aware browser due-task reminders for active tasks due today or overdue, permission-gated, ordered by due time and priority, and sent once per task per day while the app is open
- imports from Todoist, manual Google/Microsoft access-token imports, copied Nexdo task handoff briefs, plus CSV, ICS, JSON/Trello/Things-style sources, with client-side demo file imports, file previews before mutation, visible workspace hydration after authenticated import success, and task quota enforcement for authenticated imports
- Settings data export for the tasks currently loaded in the workspace as JSON or CSV, using the same structured task metadata agents need for backup, review, and handoff
- API key based MCP/ChatGPT Actions interop for listing, creating, completing, updating, adding notes to, searching, reading, and briefing tasks; agent task updates can maintain core planning metadata such as due time, action type, estimate, energy, people, tags, parent/related task links, and the full task status contract including `cancelled`; cancelled work remains visible and restorable in the human All Tasks flow
- Power/team-gated API-key access, scoped API-key permissions, rotation rate limits, prerequisite- and scope-aware MCP setup UI, a copyable bounded operating brief for external agents, and an agent action audit table/migration for MCP/API-key calls; MCP tool execution pre-creates the audit row and fails before handler mutation if audit logging is unavailable
...
- PR #3 GitHub Actions Web rails passed on current pushed product head `537c04b8b92b6fff38bb1519fe016e3fa7f0a20b` after the browser-reminder due-time ordering change. Recheck PR status after every newer push before treating the branch as current-green.
- PR #3 Vercel preview evidence is per-head: deployment passed on the inspected due-time scan-ordering head `1d12864044597dc7202ab068bc423f9ba25cf667`, a later product head hit account build-rate limiting, and a later docs-only head deployed successfully. Inspect the current PR checks instead of inferring preview state from an older hash.
...
- Agent request validation: MCP JSON-RPC should reject malformed non-object request bodies before reading JSON-RPC fields, and `tools/call` plus ChatGPT Action wrappers should reject non-object tool argument payloads before tool execution so arrays, strings, and `null` bodies cannot be silently treated as valid arguments. Keep Bearer token parsing centralized so MCP and ChatGPT Action routes accept standard case-insensitive auth schemes with harmless extra spacing while still requiring a non-empty token.
...
- Task notes: shared note validation and demo persistence live in `lib/task-notes.ts`; authenticated owned-task note routes live at `app/api/tasks/[id]/notes/route.ts`; `add_task_note` in `lib/mcp-tools.ts` is the external-agent note append path and may mark notes as `agent_result` for bounded agent-produced findings; task detail is the human-facing notes surface. Task-note metadata is column-limited by migration `008_task_note_column_grants.sql`, and server routes should write note metadata through service-role paths after ownership checks.
- Task relationships: validation lives in `lib/task-relationships.ts`; authenticated relationship writes use `app/api/tasks/[id]/relationships/route.ts` and MCP `update_task` with service-role persistence after checking all linked task IDs are owned by the same user. Keep direct browser grants for `parent_task_id` and `related_task_ids` closed unless this ownership model changes deliberately.
- Imports: source-specific and generic normalization in `lib/importers.ts`; import routes under `app/api/import/`.
- Task handoffs: `lib/task-handoff.ts` formats and parses copied `# Nexdo Task Handoff` briefs; task detail owns copy-out, and `/import` owns paste-in restore into task metadata plus reviewable notes. Paste parsing validates duplicate fields, enums, local due dates/times, estimates, text/list bounds, trace ref length, and note length before mutation. Do not write source-agent trace fields directly from browser import paths; preserve them as handoff trace notes unless the server/API-key path is explicitly doing an audited agent mutation.
- Task exports: `lib/task-export.ts` defines the portable JSON/CSV export contract used by Settings > Data; keep export fields aligned with agent-readable planning metadata when task fields change.
...
for disposable task creation, structured task and relationship update, note append, completion,
