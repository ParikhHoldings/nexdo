...
- PR #3 Web rails passed on the latest head after dependency audit was added to CI
- PR #3 Vercel preview deployment passed on earlier branch heads, but the latest head is currently blocked by Vercel's account build-rate limit, so inspect current PR checks before treating the newest head as preview-deploy verified
