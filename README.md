...
- Power/team-gated API access, scoped API-key permissions, hashed one-time-reveal API keys, restricted browser-visible profile columns, API-key rotation rate limits, scope-aware MCP setup UI, and an agent action audit table for MCP/API-key calls
...
- `npm run test:e2e` passed for the logged-out `/today` demo flow, All Tasks/Upcoming/Done demo workspace lifecycle, demo task reload persistence, and MCP/OpenAPI/action auth smoke tests
