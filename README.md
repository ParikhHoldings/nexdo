...
- imports from Todoist, manual Google/Microsoft access-token imports, and file uploads from CSV, ICS, JSON/Trello/Things-style task exports, including client-side demo file imports for logged-out visitors
...
- Power/team-gated API access, scoped API-key permissions, hashed one-time-reveal API keys, server-owned profile rows with restricted browser-visible columns and bounded preference updates, API-key rotation rate limits, prerequisite- and scope-aware MCP setup UI, and an agent action audit table for MCP/API-key calls; MCP tool execution fails before handler mutation when audit logging is unavailable
...
- PR #3 GitHub Actions Web rails passed on the latest inspected feature head `13c553d911b8648cc543645edc2a9a411dcc25c3`; inspect current checks after every newer push before treating the branch as current-green
...
- latest successful Vercel preview deployment remains head `6e196ed49a5445f404494599f2f4e7af5801d185` at `https://ph-nexdo-39vxlvbl6-nathan-happywpcos-projects.vercel.app`
