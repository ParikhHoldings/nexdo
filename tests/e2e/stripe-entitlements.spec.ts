...
test('Stripe webhook smoke verifies quota plan state after tier changes', () => {
  ...
  expect(routeSource).toContain('function stripeSubscriptionId(')
  expect(routeSource).toContain('const customerId = stripeCustomerId(session.customer)')
  expect(routeSource).toContain('const subscriptionId = stripeSubscriptionId(session.subscription)')
  ...
  expect(routeSource).toContain('const { error: eventInsertError } = await supabase')
  expect(routeSource).toContain('Failed to record Stripe event')
