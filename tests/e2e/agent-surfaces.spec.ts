...
test('MCP smoke can provision disposable scoped API keys', () => {
  ...
  expect(source).toContain('async function assertMalformedPayloadGuards')
  expect(source).toContain("await rpcNotification('notifications/initialized')")
  expect(source).toContain('await assertMalformedPayloadGuards()')
  expect(source).toContain('Invalid JSON-RPC request')
  expect(source).toContain('Tool call params must be an object')
  expect(source).toContain('Tool arguments must be a JSON object')
